...
# Руководство по использованию примеров MPP аудио-видео

## 1 Краткое описание 

Целью организации документации по использованию примеров MPP является сделать примеры более удобными в использовании. 

## 2 Введение

Примеры MPP обычно находятся в каталоге sample MPP Middleware. Кроме того, в каталоге demo фреймворка MPP также есть некоторые примеры. Этот документ в основном описывает основные методы использования различных примеров MPP Middleware: конфигурацию, компиляцию, тестирование, а также категории примеров, поддержку на различных платформах и методы тестирования и т.д. Раздел FAQ в конце документа содержит подробное описание методов тестирования функций кодирования и декодирования аудио-видео и использования инструментов тестирования.	

## 3 Конфигурация

​	Этот раздел в основном описывает метод конфигурации примеров MPP. В настоящее время существуют различия между Tina и Melis.

### 3.1 Метод конфигурации примеров MPP на Tina 

На схемах Tina V853, V833 и V536 примеры MPP поддерживают конфигурацию через menuconfig. После выбора [*] выбрать пример mpp текущие поддерживаемые примеры MPP будут отображаться, все не выбраны по умолчанию. В этом момент вы можете выбрать примеры, которые хотите протестировать, затем сохраните конфигурацию и перекомпилируйте MPP.
...
### 3.2 Метод конфигурации примеров MPP на Melis

 Melis 上各方案，目前 MPP sample 不支持从 menuconfig 中配置。若想开启或关闭 sample，只 能修改 Makefile 文件，然后重新编译 MPP。

## 4 Компиляция

Этот раздел в основном описывает метод компиляции примеров MPP. 

Ниже рассмотрены примеры Tina. 
...
Этот раздел в основном описывает методы тестирования примеров MPP.

###  5.1 Тестирование с помощью последовательного порта на плате
...
### 5.2 Тестирование с помощью adb shell на стороне ПК
...
Этот раздел в основном описывает классификацию примеров MPP. 
...
Этот раздел в основном описывает текущее состояние различных примеров MPP, то есть поддержку на различных схемах платформы Tina и Melis.

###  7.1 Поддержка примеров MPP на различных схемах платформы Tina
...
### 7.2 Поддержка примеров MPP на различных схемах платформы Melis
...
Этот раздел конкретно описывает метод тестирования каждого примера MPP. Основное содержание включает: цель тестирования, зависимости компонентов, путь тестирования, исходные файлы, целевые файлы, конфигурацию параметров, команды тестирования, выход из теста, ожидаемые результаты и т.д.
...
```
Демонстрирует прямой вызов интерфейса драйвера vin для получения видеокадров.
Этот пример не использует никаких компонентов mpp.
```
...
```
Сохраните один кадр на каждые несколько кадров в указанный каталог вывода. 
```
...
fps: 指定camera采集的帧率。
test_frame_count: 指定测试采集的frame总数，0表示Нет限。 
...
vipp12被LDC（直方图均衡）功能占用，Нет法测试，故实测时取值2。即测试前3个vipp。
注：通过修改isp配置，关闭LDC功能后，可以测试全部4个vipp。
```

Команда тестирования: [Tina] 
...
test_duration：测试时间，单位: s。0表示Нет限时长。
```
...
Group创建完后必须创建Port 0，Port 1~ Port3可以根据实际需要依次创建，Port 1~Port 3支持Нет极缩放，图像缩
...
# VO, type: [lcd]\[hdmi]
use_vo = 1 # 仅用于在线模式，控制是否开启视频实时预览（在多实例模式下该选项Нет效，强制打开）
disp_w = 1920 # 仅用于在线模式，控制视频显示的分辨率
disp_h = 1080
disp_type = "hdmi" # 仅用于在线模式，视频显示的终端类型
# VENC: default use EIS output size
use_venc = 0 # 在多实例模式下该选项Нет效，强制打开，离线仿真模式下该选项使用 EIS_BYPASS_VENC 来控
制。
...
src_rect_w =
src_rect_h =
设定，注意：若Нет效启动rotation, dst_rotate = 0
4) pixel format格式转换
该功能通过参数：
pic_format = nv21
dst_format = nv12
...
test_duration：指定测试时间，单位：秒。0表示Нет限时长。
```
...
display_width:显示宽度
display_height:显示高度
test_frame_count:测试帧数，0代表Нет限帧数。
brightness:uvc亮度设置
contrast:uvc对比度设置
hue:uvc色度设置
saturation:uvc饱和度设置
...
```
图片Исходные файлы:nna_g2d/c1080.jpg
```
...
test_duration: 测试时间，0表示Нет限。单位：秒。
```
...
使用 Elescard StreamEye 4.6 工具（只支持 H264），按如下截图配置后，开启 P 帧帧内刷 新后，图像看到一个橙色的竖状矩形条，同时逐帧往后查看时矩形条会从左往右移动；关闭 P 帧 帧内刷新后，则Нет此矩形条。
...
使用 YUView 工具（需要配置 ffmpeg 动态库）可分析 H265 文件，勾选 Pred Mode 后，可显 示一个蓝色的竖状矩形条，同时逐帧往后查看时矩形条会从左往右移动；关闭 P 帧帧内刷新后， 则Нет此矩形条。
...
使用Примечание： 检测采集通道的数据和回采通道的数据的延迟关系是否正确。 需要特殊的正弦波文件 tone_16k_accurate.wav，tone_8k_accurate.wav。 一般采集通道的数据应该稍微延迟一些。
